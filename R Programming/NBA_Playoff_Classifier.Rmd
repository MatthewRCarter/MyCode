...
#loading data
singe_player_data <-read_xlsx("C:/Users/Matthew/Documents/STAT_434/Final_Project/single_player_data_2.xlsx")
```
...
playoff_team_data <-read_xlsx("C:/Users/Matthew/Documents/STAT_434/Final_Project/Team_Playoff_Data.xlsx")
...
```{r}
clutch <- read_xlsx("C:/Users/Matthew/Documents/STAT_434/Final_Project/clutch.xlsx")
clutch <- clutch[,1:3]
# names(clutch) <- c()
```
...
team_name_fix(clutch)
team_name_fix(playoff_data)
```
...
```{r}
#QDA full model
QDAfunctfull <- function(train, test){

qda.fit <- qda(Playoffs_1~.,data=quant_only ,subset = train)
qda.fit
...
correct_perc <- list()
for (i in 1:length(classes)){
correct_perc[i] <- mean(predicted_playoff[predicted_playoff == classes[i]] == actual_playoff[predicted_playoff == classes[i]])
}

return (list(qdatable, correct_perc)) # Returns a list of list, be wary when calling things

}

QDAfunctfull(train_3,test_3) # Cannot compare to the full model becasue there are too many predictors compared to the number of observations in each class of the response variable
```
...
```{r}
setwd("C:/Users/Matthew/Documents/GSB 510/Final_Presentation")
